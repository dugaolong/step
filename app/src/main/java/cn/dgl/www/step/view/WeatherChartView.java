...
//            }
    ...
    private void drawText(Canvas canvas, Paint textPaint, int i, int[] temp, float[] yAxis, int type) {
        switch (type) {
            case 0:
                // 显示白天气温
                canvas.drawText(temp[i] + "°C", mXAxis[i], yAxis[i] - mRadius - mTextSpace, textPaint);
                break;
            case 1:
                // 显示夜间气温
                canvas.drawText(temp[i] + "°C", mXAxis[i], yAxis[i] + mTextSpace + mTextSize, textPaint);
                break;
        }
    }
    ...
    private void setHeightAndXAxis() {
        Log.e("getHeight()","getHeight()==="+getHeight());
        mHeight = getHeight();
        // 控件宽
        int width = getWidth();
        // 每一份宽
